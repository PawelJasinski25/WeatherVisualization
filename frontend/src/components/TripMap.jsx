...
const createRepeatingGradient = (stops, repeats) => [
    "interpolate", ["linear"], ["%", ["*", ["line-progress"], repeats], 1], ...stops.slice(3)
];
...
const TripMap = ({ tripId }) => {
    ...
    const segmentsData = useMemo(() => {
        ...
        tripData.forEach(d => {
            ...
            if (lastPt) {
                // Tniemy TYLKO i wyłączenie wtedy, gdy zmienia się segmentId z bazy danych
                if (segmentId !== undefined && lastPt[2] !== undefined && segmentId !== lastPt[2]) {
                    cut = true;
                }
            }
        ...
        return allSegments.map((seg, idx) => {
            ...
            return {
                id: `segment-${idx}`,
                geojson: { type: "Feature", geometry: { type: "LineString", coordinates: seg } },
                windGradient: createRepeatingGradient(windStops, NUM_REPEATS),
                tempGradient: createRepeatingGradient(tempStops, NUM_REPEATS)
            };
        });
    }, [tripData]);
            ...
            >
                {segmentsData.map(seg => (
                    <Source key={seg.id} id={seg.id} type="geojson" data={seg.geojson} lineMetrics>
                        <Layer id={`${seg.id}-wind-contour`} type="line" paint={{ "line-color": "#213BD4", "line-width": 9, "line-offset": -2.5 }} layout={{ "line-join": "round", "line-cap": "round" }} />
                        <Layer id={`${seg.id}-temp-contour`} type="line" paint={{ "line-color": "#f8760d", "line-width": 9, "line-offset": 2.5 }} layout={{ "line-join": "round", "line-cap": "round" }} />
                        <Layer id={`${seg.id}-wind`} type="line" paint={{ "line-width": 5, "line-offset": -2.5, "line-gradient": seg.windGradient }} layout={{ "line-join": "round", "line-cap": "round" }} />
                        <Layer id={`${seg.id}-temp`} type="line" paint={{ "line-width": 5, "line-offset": 2.5, "line-gradient": seg.tempGradient }} layout={{ "line-join": "round", "line-cap": "round" }} />
                    </Source>
                ))}
            </Map>
        </div>
