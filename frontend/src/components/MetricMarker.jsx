...
const MetricMarker = ({ pt, metricId, index }) => {
    ...
    const routeBearing = pt.routeBearing || 0;
    const mathRad = (90 - routeBearing) * (Math.PI / 180);
    ...
    const distancePx = isArrow ? 26 : 44;
    const perpX = Math.sin(mathRad) * distancePx;
    const perpY = Math.cos(mathRad) * distancePx;
    ...
    if (isArrow) {
        ...
        return (
            <Marker longitude={pt.lng} latitude={pt.lat} offset={[dx, dy]} anchor="center">
                ...
                >
                    <path d="M3 3 L22 12 L3 21 L8 12 Z" fill={themeColor} stroke={themeColor} strokeWidth={outlineWidth} strokeLinejoin="round" />
                </svg>
            </Marker>
        );
    } else {
